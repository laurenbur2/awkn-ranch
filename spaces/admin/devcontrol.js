...
async function loadContext() {
  ...
  const CONTEXT_FILES = [
    ...
    { name: 'Project CLAUDE.md', path: './CLAUDE.md', category: 'instructions', gh: 'CLAUDE.md',
      desc: 'Project-specific directives and code guards for AWKN Team Portal. Defines mandatory behaviors (version stamping, push-on-change, CI versioning), code guards (media_spaces naming, showToast, Tailwind aap-* tokens, hero banner protection), on-demand doc loading triggers, and quick refs for the tech stack (Vanilla HTML/JS, Tailwind v4, Supabase, GitHub Pages, Capacitor 8).' },
...
async function loadBackups() {
  ...
  try {
    ...
    const services = [
      ...
      { key: 'r2',       icon: '☁', label: 'Cloudflare R2', desc: 'S3 sync your-app bucket' },
      { key: 'd1',       icon: '📋', label: 'Cloudflare D1', desc: 'claude-sessions export' },
      { key: 'github',   icon: '🔀', label: 'GitHub Repo', desc: 'Bare mirror of USERNAME/REPO' },
    ];
    ...
    panel.innerHTML = `
      ...
      <div style="margin-bottom:1.5rem;">
        <p style="font-size:0.75rem;color:var(--text-muted,#aaa);">
          <strong>Storage:</strong> /Volumes/RVAULT20/backups/your-app/ · <strong>Cron:</strong> Almaca · <strong>Retention:</strong> 12 DB dumps, 12 D1 exports, full R2 mirror, bare Git mirror
        </p>
      </div>
