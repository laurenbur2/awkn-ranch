...
> Updated 2026-05-04 (Phase 1 Pass 6). Prod project: `lnqxarwqckpmirpmixcw` (West US Oregon). Read-only introspection: `supabase db query --linked`. **Zero prod writes during Phase 1.**

> **Drift warning:** Per the 2026-05-04 prod audit (`docs/superpowers/work/2026-05-04-prod-db-audit.md`), prod has 70 public tables — all AWKN/Within Center. Several tables documented historically (`signwell_config`, `rental_applications`, `lease_templates`, `event_hosting_requests`, `voice_assistants`, `voice_calls`, `vehicles`, `weather_config`, all IoT tables) **do not exist in prod**. Code referencing them fails empirically. Schema reconciliation lands in the relevant Next.js phase (2–6), not Phase 1.
